...
export const fetchUsers = createAsyncThunk(
    ...
    async ({ token, page = 0, size = 9 }, { rejectWithValue }) => {
        try {
            const response = await axios.get(`http://localhost:8080/api/users?page=${page}&size=${size}`, {
                headers: {
                    Authorization: `Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJoZXdlcmk3MjI5QGFsbWF4ZW4uY29tIiwiaWF0IjoxNzI1MTM1NzE1LCJleHAiOjE3MjUyMjIxMTV9.mvuorq23m31mHlpds9_hWZ8lCX9ofLsKlTFiopKwWn4`,
                },
            });
            return response.data;
        } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchUsers.fulfilled, (state, action) => {
                state.loading = false;
                state.users = action.payload; // Assuming the payload has the full data structure
            })
